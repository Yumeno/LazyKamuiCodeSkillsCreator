---
name: i2i-kamui-reve-remix
description: MCP skill for i2i-kamui-reve-remix. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-reve-remix tools that require async processing.
---

# i2i-kamui-reve-remix

MCP integration for **i2i-kamui-reve-remix**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/reve/remix
```
...
1. **Submit**: `reve_remix_submit` - Start job, returns request_id
2. **Status**: `reve_remix_status` - Poll job status with request_id
3. **Result**: `reve_remix_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-reve-remix.yaml`.

**Quick reference** (name and description only):

- **reve_remix_result**: Get result of completed Reve Remix generation. Returns raw JSON response from Fal.ai API.
- **reve_remix_status**: Check status of Reve Remix processing using request ID.
- **reve_remix_submit**: Submit Fal.ai Reve Remix request to remix images by combining text prompts with reference images (1-4 images) and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-reve-remix.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-reve-remix.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-reve-remix.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-reve-remix/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-reve-remix.yaml`
