---
name: i2i-kamui-image2pixel
description: MCP skill for i2i-kamui-image2pixel. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-image2pixel tools that require async processing.
---

# i2i-kamui-image2pixel

MCP integration for **i2i-kamui-image2pixel**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/image2pixel
```
...
1. **Submit**: `image2pixel_i2i_submit` - Start job, returns request_id
2. **Status**: `image2pixel_i2i_status` - Poll job status with request_id
3. **Result**: `image2pixel_i2i_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-image2pixel.yaml`.

**Quick reference** (name and description only):

- **image2pixel_i2i_result**: Get result of completed Image2Pixel pixel art conversion.
- **image2pixel_i2i_status**: Check status of Image2Pixel pixel art conversion using request ID.
- **image2pixel_i2i_submit**: Submit Image2Pixel conversion request to convert images into pixel art style and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-image2pixel.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-image2pixel.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-image2pixel.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-image2pixel/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-image2pixel.yaml`
