---
name: t2i-kamui-flux-2-pro
description: MCP skill for t2i-kamui-flux-2-pro. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-flux-2-pro tools that require async processing.
---

# t2i-kamui-flux-2-pro

MCP integration for **t2i-kamui-flux-2-pro**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/flux-2-pro
```
...
1. **Submit**: `flux_2_pro_submit` - Start job, returns request_id
2. **Status**: `flux_2_pro_status` - Poll job status with request_id
3. **Result**: `flux_2_pro_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-flux-2-pro.yaml`.

**Quick reference** (name and description only):

- **flux_2_pro_result**: Get result and download completed Flux-2 Pro generated image to local directory.
- **flux_2_pro_status**: Check status of Flux-2 Pro image generation processing using request ID.
- **flux_2_pro_submit**: Submit Fal.ai Flux-2 Pro text-to-image generation request and receive request ID for polling. Generate high-quality images from text prompts.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-flux-2-pro.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-flux-2-pro.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-flux-2-pro.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-flux-2-pro/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-flux-2-pro.yaml`
