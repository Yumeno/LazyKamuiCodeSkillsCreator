---
name: i2i-kamui-qwen-image-edit
description: MCP skill for i2i-kamui-qwen-image-edit. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-qwen-image-edit tools that require async processing.
---

# i2i-kamui-qwen-image-edit

MCP integration for **i2i-kamui-qwen-image-edit**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/qwen/image-edit
```
...
1. **Submit**: `qwen_image_edit_submit` - Start job, returns request_id
2. **Status**: `qwen_image_edit_status` - Poll job status with request_id
3. **Result**: `qwen_image_edit_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-qwen-image-edit.yaml`.

**Quick reference** (name and description only):

- **qwen_image_edit_result**: Get the result and download completed Qwen Image Edit images.
- **qwen_image_edit_status**: Check the status of a Qwen Image Edit processing request.
- **qwen_image_edit_submit**: Submit a Qwen Image Edit request. Edits and transforms input images using AI-powered text prompts.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-qwen-image-edit.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-qwen-image-edit.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-qwen-image-edit.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-qwen-image-edit/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-qwen-image-edit.yaml`
