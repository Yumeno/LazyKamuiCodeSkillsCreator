---
name: t2i-kamui-fal-longcat-image
description: MCP skill for t2i-kamui-fal-longcat-image. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-fal-longcat-image tools that require async processing.
---

# t2i-kamui-fal-longcat-image

MCP integration for **t2i-kamui-fal-longcat-image**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/longcat-image
```
...
1. **Submit**: `longcat_image_submit` - Start job, returns request_id
2. **Status**: `longcat_image_status` - Poll job status with request_id
3. **Result**: `longcat_image_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-fal-longcat-image.yaml`.

**Quick reference** (name and description only):

- **longcat_image_result**: Get result of completed LongCat Image text-to-image generation.
- **longcat_image_status**: Check status of LongCat Image text-to-image processing using request ID.
- **longcat_image_submit**: Submit Fal.ai LongCat Image text-to-image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-fal-longcat-image.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-fal-longcat-image.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-fal-longcat-image.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-fal-longcat-image/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-fal-longcat-image.yaml`
