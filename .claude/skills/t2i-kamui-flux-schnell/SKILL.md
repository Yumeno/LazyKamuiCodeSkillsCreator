---
name: t2i-kamui-flux-schnell
description: MCP skill for t2i-kamui-flux-schnell. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-flux-schnell tools that require async processing.
---

# t2i-kamui-flux-schnell

MCP integration for **t2i-kamui-flux-schnell**.
...
1. **Submit**: `flux_schnell_submit` - Start job, returns request_id
2. **Status**: `flux_schnell_status` - Poll job status with request_id
3. **Result**: `flux_schnell_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-flux-schnell.yaml`.

**Quick reference** (name and description only):

- **flux_schnell_result**: Get result and download completed Flux-1 Schnell text-to-image to local directory.
- **flux_schnell_status**: Check status of Flux-1 Schnell text-to-image processing using request ID.
- **flux_schnell_submit**: Submit Fal.ai Flux-1 Schnell text-to-image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-flux-schnell.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-flux-schnell.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-flux-schnell.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-flux-schnell/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-flux-schnell.yaml`
