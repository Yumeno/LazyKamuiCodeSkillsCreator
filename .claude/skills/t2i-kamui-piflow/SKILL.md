---
name: t2i-kamui-piflow
description: MCP skill for t2i-kamui-piflow. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-piflow tools that require async processing.
---

# t2i-kamui-piflow

MCP integration for **t2i-kamui-piflow**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/piflow
```
...
1. **Submit**: `piflow_submit` - Start job, returns request_id
2. **Status**: `piflow_status` - Poll job status with request_id
3. **Result**: `piflow_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-piflow.yaml`.

**Quick reference** (name and description only):

- **piflow_result**: Get result of completed Piflow Text-to-Image generation.
- **piflow_status**: Check status of Piflow Text-to-Image processing using request ID.
- **piflow_submit**: Submit Piflow (π-Qwen) Text-to-Image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-piflow.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-piflow.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-piflow.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-piflow/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-piflow.yaml`
