---
name: t2i-kamui-nano-banana
description: MCP skill for t2i-kamui-nano-banana. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-nano-banana tools that require async processing.
---

# t2i-kamui-nano-banana

MCP integration for **t2i-kamui-nano-banana**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/nano-banana
```
...
1. **Submit**: `nano_banana_t2i_submit` - Start job, returns request_id
2. **Status**: `nano_banana_t2i_status` - Poll job status with request_id
3. **Result**: `nano_banana_t2i_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-nano-banana.yaml`.

**Quick reference** (name and description only):

- **nano_banana_t2i_result**: Get the result and download completed Nano Banana generated images with AI-generated descriptions.
- **nano_banana_t2i_status**: Check the status of a Nano Banana Text-to-Image processing request.
- **nano_banana_t2i_submit**: Submit Nano Banana Text-to-Image request using Google's state-of-the-art Gemini model for high-quality image generation.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-nano-banana.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-nano-banana.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-nano-banana.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-nano-banana/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-nano-banana.yaml`
