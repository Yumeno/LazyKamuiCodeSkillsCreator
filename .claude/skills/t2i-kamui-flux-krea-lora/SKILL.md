---
name: t2i-kamui-flux-krea-lora
description: MCP skill for t2i-kamui-flux-krea-lora. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-flux-krea-lora tools that require async processing.
---

# t2i-kamui-flux-krea-lora

MCP integration for **t2i-kamui-flux-krea-lora**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/flux-krea-lora
```
...
1. **Submit**: `flux_krea_lora_t2i_submit` - Start job, returns request_id
2. **Status**: `flux_krea_lora_t2i_status` - Poll job status with request_id
3. **Result**: `flux_krea_lora_t2i_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-flux-krea-lora.yaml`.

**Quick reference** (name and description only):

- **flux_krea_lora_t2i_result**: Get result and download completed FLUX Krea LoRA Text-to-Image to local directory.
- **flux_krea_lora_t2i_status**: Check status of FLUX Krea LoRA Text-to-Image processing using request ID.
- **flux_krea_lora_t2i_submit**: Submit FLUX Krea LoRA Text-to-Image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-flux-krea-lora.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-flux-krea-lora.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-flux-krea-lora.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-flux-krea-lora/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-flux-krea-lora.yaml`
