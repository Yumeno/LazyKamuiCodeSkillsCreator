---
name: t2i-kamui-flux-2-turbo
description: MCP skill for t2i-kamui-flux-2-turbo. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-flux-2-turbo tools that require async processing.
---

# t2i-kamui-flux-2-turbo

MCP integration for **t2i-kamui-flux-2-turbo**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/flux-2/turbo
```
...
1. **Submit**: `flux_2_turbo_submit` - Start job, returns request_id
2. **Status**: `flux_2_turbo_status` - Poll job status with request_id
3. **Result**: `flux_2_turbo_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-flux-2-turbo.yaml`.

**Quick reference** (name and description only):

- **flux_2_turbo_result**: Get result of completed Flux-2 Turbo text-to-image generation. Returns JSON response with generated image URL and metadata.
- **flux_2_turbo_status**: Check status of Flux-2 Turbo text-to-image processing using request ID.
- **flux_2_turbo_submit**: Submit Fal.ai Flux-2 Turbo text-to-image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-flux-2-turbo.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-flux-2-turbo.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-flux-2-turbo.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-flux-2-turbo/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-flux-2-turbo.yaml`
