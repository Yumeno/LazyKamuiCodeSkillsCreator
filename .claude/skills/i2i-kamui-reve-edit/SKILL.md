---
name: i2i-kamui-reve-edit
description: MCP skill for i2i-kamui-reve-edit. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-reve-edit tools that require async processing.
---

# i2i-kamui-reve-edit

MCP integration for **i2i-kamui-reve-edit**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/reve/edit
```
...
1. **Submit**: `reve_edit_submit` - Start job, returns request_id
2. **Status**: `reve_edit_status` - Poll job status with request_id
3. **Result**: `reve_edit_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-reve-edit.yaml`.

**Quick reference** (name and description only):

- **reve_edit_result**: Get result of completed Reve Edit generation. Returns raw JSON response from Fal.ai API.
- **reve_edit_status**: Check status of Reve Edit processing using request ID.
- **reve_edit_submit**: Submit Fal.ai Reve Edit request to edit an image based on a text description and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-reve-edit.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-reve-edit.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-reve-edit.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-reve-edit/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-reve-edit.yaml`
