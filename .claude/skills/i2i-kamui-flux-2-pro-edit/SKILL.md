---
name: i2i-kamui-flux-2-pro-edit
description: MCP skill for i2i-kamui-flux-2-pro-edit. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-flux-2-pro-edit tools that require async processing.
---

# i2i-kamui-flux-2-pro-edit

MCP integration for **i2i-kamui-flux-2-pro-edit**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/flux-2-pro/edit
```
...
1. **Submit**: `flux_2_pro_edit_submit` - Start job, returns request_id
2. **Status**: `flux_2_pro_edit_status` - Poll job status with request_id
3. **Result**: `flux_2_pro_edit_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-flux-2-pro-edit.yaml`.

**Quick reference** (name and description only):

- **flux_2_pro_edit_result**: Get result and download completed Flux-2 Pro Edit image editing to local directory.
- **flux_2_pro_edit_status**: Check status of Flux-2 Pro Edit image editing processing using request ID.
- **flux_2_pro_edit_submit**: Submit Fal.ai Flux-2 Pro Edit image editing request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-flux-2-pro-edit.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-flux-2-pro-edit.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-flux-2-pro-edit.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-flux-2-pro-edit/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-flux-2-pro-edit.yaml`
