---
name: t2i-kamui-qwen-image
description: MCP skill for t2i-kamui-qwen-image. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-qwen-image tools that require async processing.
---

# t2i-kamui-qwen-image

MCP integration for **t2i-kamui-qwen-image**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/qwen-image
```
...
1. **Submit**: `qwen_image_t2i_submit` - Start job, returns request_id
2. **Status**: `qwen_image_t2i_status` - Poll job status with request_id
3. **Result**: `qwen_image_t2i_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-qwen-image.yaml`.

**Quick reference** (name and description only):

- **qwen_image_t2i_result**: Get result of completed Qwen Image Text-to-Image generation.
- **qwen_image_t2i_status**: Check status of Qwen Image Text-to-Image processing using request ID.
- **qwen_image_t2i_submit**: Submit Qwen Image Text-to-Image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-qwen-image.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-qwen-image.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-qwen-image.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-qwen-image/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-qwen-image.yaml`
