---
name: t2i-kamui-flux-2-flash
description: MCP skill for t2i-kamui-flux-2-flash. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling t2i-kamui-flux-2-flash tools that require async processing.
---

# t2i-kamui-flux-2-flash

MCP integration for **t2i-kamui-flux-2-flash**.

## Endpoint

```
https://kamui-code.ai/t2i/fal/flux-2/flash
```
...
1. **Submit**: `flux_2_flash_submit` - Start job, returns request_id
2. **Status**: `flux_2_flash_status` - Poll job status with request_id
3. **Result**: `flux_2_flash_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/t2i-kamui-flux-2-flash.yaml`.

**Quick reference** (name and description only):

- **flux_2_flash_result**: Get result and download completed Flux-2 Flash generated image to local directory.
- **flux_2_flash_status**: Check status of Flux-2 Flash text-to-image processing using request ID.
- **flux_2_flash_submit**: Submit Fal.ai Flux-2 Flash text-to-image generation request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/t2i-kamui-flux-2-flash.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/t2i-kamui-flux-2-flash.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/t2i-kamui-flux-2-flash.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/t2i-kamui-flux-2-flash/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/t2i-kamui-flux-2-flash.yaml`
