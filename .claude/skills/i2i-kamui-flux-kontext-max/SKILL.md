---
name: i2i-kamui-flux-kontext-max
description: MCP skill for i2i-kamui-flux-kontext-max. Provides async job execution with submit/status/result pattern via JSON-RPC 2.0. Use when calling i2i-kamui-flux-kontext-max tools that require async processing.
---

# i2i-kamui-flux-kontext-max

MCP integration for **i2i-kamui-flux-kontext-max**.

## Endpoint

```
https://kamui-code.ai/i2i/fal/flux/kontext/max
```
...
1. **Submit**: `flux_kontext_max_submit` - Start job, returns request_id
2. **Status**: `flux_kontext_max_status` - Poll job status with request_id
3. **Result**: `flux_kontext_max_result` - Get result when completed
...
> Before executing any tool, you MUST read the full specification from `references/tools/i2i-kamui-flux-kontext-max.yaml`.

**Quick reference** (name and description only):

- **flux_kontext_max_result**: Get result and download completed Flux Kontext Max images to local directory.
- **flux_kontext_max_status**: Check status of Flux Kontext Max processing using request ID.
- **flux_kontext_max_submit**: Submit Fal.ai Flux Pro Kontext Max Image-to-Image request and receive request ID for polling.

### How to Use Tools

1. **Read tool specification**: Use Read tool on `references/tools/i2i-kamui-flux-kontext-max.yaml`
...
> Before executing any tool, **read the full specification** from `references/tools/i2i-kamui-flux-kontext-max.yaml`.
> The YAML file contains `_usage` section with execution examples and CLI options.

```bash
# Read tool specification first
cat references/tools/i2i-kamui-flux-kontext-max.yaml

# Then execute (example from _usage.bash in YAML)
python .claude/skills/i2i-kamui-flux-kontext-max/scripts/mcp_async_call.py --help
```

## References

- Tool Specs & Usage: `references/tools/i2i-kamui-flux-kontext-max.yaml`
