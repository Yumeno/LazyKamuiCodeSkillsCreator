...
- 実行時（`mcp_async_call.py`）で `--config` から読み込んだヘッダー値に含まれるプレースホルダーを検出し、環境変数 → `.env` ファイルの順で解決
...
[生成時] generate_skill.py
    ...
    ├── tools/*.yaml (_usage)  → --config references/mcp.json を指定
    └── wrapper.py DEFAULTS    → ("--config", "references/mcp.json")
    ↓
[実行時] wrapper.py → mcp_async_call.py --config references/mcp.json
    ↓ main() → load_mcp_config() → _extract_headers_from_config() → resolve_header_placeholders()
...
`--config` (または自動探索) から MCP 設定を読み込み、ヘッダーを抽出・解決します:
...
#### `generate_wrapper_script()` の変更

ラッパースクリプトの DEFAULTS から `--header` を削除し、`--config` で `references/mcp.json` を指定するよう変更:
...
# → mcp_async_call.py の main() 内で load_mcp_config() → _extract_headers_from_config() → resolve_header_placeholders() が解決
```
...
| ファイル | 変更種別 | 変更内容 |
|---------|---------|---------|
| `scripts/mcp_async_call.py` | 修正 | 正規表現、.env パーサー、`resolve_header_placeholders()`、`_find_mcp_config()`、`_extract_headers_from_config()`、`main()` のヘッダー処理書き換え |
...
5. `main()` で `--config` からヘッダーを自動抽出し `resolve_header_placeholders()` で解決
...
- YAML の `_usage.bash` に `--config` が含まれること
- ラッパースクリプトの DEFAULTS に `("--config", ...)` が含まれること
...
# 実行（--config からヘッダーが自動抽出され、プレースホルダーが解決される）
...
# 環境変数は未設定の状態で実行（--config からヘッダー自動読み込み）
unset TEST_KEY
python mcp_async_call.py --config references/mcp.json ...
```
...
```bash
# 環境変数未設定、.env なし（--config 内のヘッダーに未解決プレースホルダーがある場合）
