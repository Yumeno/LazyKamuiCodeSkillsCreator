...
Claude Code のプロンプトで以下のように指示する:

```
以下の3つの issue をサブエージェント (issue-fixer) で並列に修正してください。
各サブエージェントはワークツリーで分離し、バックグラウンドで実行してください。
完了後にそれぞれ PR を作成してください。

- #21: --id-param 注入バグ + headers 重複修正
- #22: ワーカー自動起動
- #23: 認証キーハードコード

#24 と #25 は worker.py のレスポンス整形が競合するので、
上記3つの PR がマージされてから順番に対応してください。
```

カスタムエージェントを作成していない場合は、プロンプトで直接指示:

```
以下の3つの issue をサブエージェントで並列に修正してください。
各サブエージェントはワークツリー分離で、バックグラウンド実行してください。
...
2. fix/<slug> ブランチを作成
3. コードを修正・テスト
4. コミット → プッシュ → gh pr create で PR 作成
```
